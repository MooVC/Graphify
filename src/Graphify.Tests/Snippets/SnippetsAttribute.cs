...
using Xunit.Sdk;
using static Graphify.Snippets.Extensions;
    ...
    : DataAttribute
{
    ...
    private static Type[] FindDeclarations()
    {
        return Assembly
            ...
            .GetTypes()
            .Where(type => type.Namespace is not null && type.Namespace.StartsWith("Graphify.Snippets.Declarations", StringComparison.InvariantCulture))
            .ToArray();
    }
