namespace Graphify.Strategies
{
    ...
    /// <summary>
    /// Provides an asynchronous strategy for generating contents that match each tier within the hierarchy that involve a sequence.
    /// </summary>
    ...
    where TInstance : class
{
    return global::Graphify.NavigatorExtensions.Invoke(instance, observers, cancellationToken);
        ...
        public IEnumerable<Source> Generate(Subject subject)
        {
            ...
            foreach (Source source in GenerateCore(subject))
            {
                yield return source;
            }
        }
        ...
        private static IEnumerable<Source> GenerateContentForElement(string @class, Element element, string @namespace, string method, Predecessor[] preceding, Property property, Subject subject, int tier)
        {
            ...
            try
            {
                ...
                yield return GenerateContent(arguments, body.Indent(skip: 0, times: 2, trim: false), @class, element, element.Name, @namespace, moniker, parameters, subject, GenerateContentForElementContent, property.Type, out string next);
        ...
        private static Source GenerateContentForProperty(string arguments, string @class, string @namespace, string method, string parameters, Property property, Subject subject, int tier, out string next)
        {
            ...
            return GenerateContent(arguments, body.Indent(skip: 0, trim: false), @class, property.Element, property.Name, @namespace, method, parameters, subject, GenerateContentForPropertyContent, property.Type, out next);
        }
        ...
        private static string GenerateConcatenations(string call, Element element, string method, ImmutableArray<Property> properties, string template)
        {
            ...
            {
                _ = builder.AppendLine(string.Format(GenerateConcatenationsForPropertyElement, string.Concat(method, element.Name), call, element.Name));
            }
